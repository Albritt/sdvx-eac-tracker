...
def parse_song_results(text: str, domain_name: str, headers: dict) -> list[dict[str, str|dict]]:
    ...
    if soup.find(class_="music"):
       for tag in soup.find_all(class_="music"):
           ...
           subpage = get_song_subpage(song_data['music_id_url'], domain_name, headers)
           jacket_urls = get_song_jacket_urls(subpage, domain_name)
           add_jackets_to_charts(song_data['charts'],jacket_urls)
...
def get_song_metadata(tag: Tag) -> dict[str, Any]:
    ...
    music_id_url = get_music_id_url(tag)
...
def get_charts(tag: Tag) -> dict[str, dict[str, int]]:
    ...
    for p in charts_tag.find_all("p"):
        diff_name = p["class"][0].upper()
        chart_level = int(p.get_text(strip=True))
        charts[diff_name] = {'level' : chart_level}
    return charts
...
def request_jacket(img_url) -> bytes|Any:
    ...
    if response.status_code == 200:
        ...
        print(f"Unable to download for: {img_url}")
